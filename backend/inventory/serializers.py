...
class StockMovementSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        logger.warning("=" * 50)
        logger.warning("🔍 DEBUG StockMovementSerializer.create()")
        ...
        if request and hasattr(request, 'user'):
            ...
            if request.user.is_authenticated:
                ...
                logger.warning(f"✅ Utilisateur authentifié: {request.user.username}")
            else:
                logger.warning("⚠️ Utilisateur non authentifié, utilisation admin par défaut")
                # Utiliser un admin par défaut
                from django.contrib.auth import get_user_model
                User = get_user_model()
                admin_user = User.objects.filter(is_superuser=True).first()
                if admin_user:
                    validated_data['user'] = admin_user
                    logger.warning(f"✅ Admin par défaut: {admin_user.username}")
                else:
                    raise serializers.ValidationError({"user": ["Aucun admin trouvé"]})
        else:
            logger.warning("⚠️ Pas de request ou user, utilisation admin par défaut")
            # Utiliser un admin par défaut
            from django.contrib.auth import get_user_model
            User = get_user_model()
            admin_user = User.objects.filter(is_superuser=True).first()
